...
-- vote_rooms
-- ----------
-- Anyone can read active rooms; creators (auth or session via created_by) can read their own.
...
-- Only authenticated users can create rooms tied to themselves.
...
-- Owners can update/deactivate their rooms
...
-- ----------
-- Anyone may insert a vote (anonymous allowed). Consider adding rate limiting at the API.
create policy "votes: anyone can insert"
on public.votes
for insert
with check (true);

-- Read votes only for active rooms or the owner of the room (via join)
...
for select
using (exists (
  select 1 from public.battle_arguments a
